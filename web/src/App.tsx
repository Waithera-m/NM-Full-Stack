...
const AddNewPayment = ({onSuccess}: UpdateLoanPaymentProps) => {
    ...
    const submitPayment = (form: HTMLFormElement) => {
        ...
        const loan_id = Number(formData.get('loan-id')); //Get passed loan id
        const amount = Number(formData.get('payment-amount')); //Get passed payment amount

        console.log('Submitting Payment::::', {loan_id, amount});

        /**Send payment update result to the backend */
        fetch('http://localhost:2024/rest-update-loan-payment', {
            method: 'POST',
            headers: {
